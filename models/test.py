...
from utils.inc_net import HeadNet, MLPHead
...
convnet_path = "cifat100_50_resnet18.pkl"  # path of the pretrained convnet
...
class Test(BaseLearner):
    def __init__(self, args):
        super().__init__(args)
        self.args = args
        self._network = HeadNet(args, False, MLPHead(512))
    ...
    def _train(self, train_loader, test_loader):
        ...
        if self._cur_task == 0:
            self._network.load_state_dict(torch.load(convnet_path, map_location=self._device)["model_state_dict"],
                                          strict=False)
